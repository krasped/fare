...
const TableColumnFilter: FC<Props> = ({ column, table }) => {
  ...
  if (column.id === "dateOfBirth") {
    ...
    return (
      <DesktopDatePicker
        ...
        value={columnFilterValue ?? ""}
        renderInput={(params) => (
          <TextField {...params} sx={DATE_PICKER_TEXT_FIELD_STYLE} />
        )}
      />
    );
  }
  ...
  return (
    <StyledSearchInput
      value={columnFilterValue ?? ""}
      onChange={(e) => column.setFilterValue(e.target.value)}
      endAdornment={<StyledSearchIcon sx={{ color: "text.secondary" }} />}
      sx={{ backgroundColor: "divider", borderRadius: "8px" }}
    />
  );
};
